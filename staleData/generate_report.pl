...
$phedex_ref -> get( ("http://cmsweb.cern.ch/phedex/datasvc/perl/prod/blockreplicas?node=$site_name;dataset=*"));
...
#		print $datasetName[0], "\n";
		...
		for my $replica (@{$block->{REPLICA}}){
			...
			if (!exists $datasets {$datasetName[0]}{setsize}){$datasets{$datasetName[0]}{setsize}=0; $setsize = 0;}
